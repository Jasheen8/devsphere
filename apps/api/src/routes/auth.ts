...
r.post("/register", async (req, res) => {
  ...
  if (!p.success)
    return res
      .status(400)
      .json({ error: "Invalid input", details: p.error.flatten() });
  const email = p.data.email.trim().toLowerCase();
  const { password, name } = p.data;
  ...
  setAuthCookie(res, makeToken(user.id));
  res.status(201).json({
    user: {
      id: user.id,
      email: user.email,
...
r.post("/verify/request", async (req, res) => {
  ...
  if (user) {
    ...
    await emailProvider.send(
      user.email,
      "Verify your Devsphere account",
      `Verify your account with token: ${token}`,
    );
  }
  res.json({ ok: true });
});
r.post("/verify/confirm", async (req, res) => {
...
r.post("/password/request", async (req, res) => {
  const p = z.object({ email: z.string().email() }).safeParse(req.body);
  if (!p.success) return res.status(400).json({ error: "Invalid email" });
  const email = p.data.email.trim().toLowerCase();
  ...
  if (user) {
    ...
    await emailProvider.send(
      user.email,
      "Reset your Devsphere password",
      `Reset token: ${token}`,
    );
  }
  res.json({ ok: true });
});
r.post("/password/confirm", async (req, res) => {
  const p = z
  ...
  await db.$transaction([
    db.user.update({
      where: { id: row.userId },
      data: { passwordHash: await hashPassword(p.data.password) },
    }),
    db.emailToken.update({
      where: { id: row.id },
      data: { usedAt: new Date() },
...
r.post("/logout", auth, async (_req, res) => {
  clearAuthCookie(res);
  res.status(204).end();
});
